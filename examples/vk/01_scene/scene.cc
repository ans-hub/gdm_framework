...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, LPWSTR cmdLine, int cmdShow)
{
  ...
  uint depth_idx = 1;
  uint input_idx = -1;
  render_pass.SetAttachmentDesc(color_idx, gfx.GetSurfaceFormat(), gfx::EImageLayout::COLOR_ATTACHMENT_OPTIMAL);
  render_pass.SetAttachmentDesc(depth_idx, depth_image.GetFormat<gfx::EFormatType>(), gfx::EImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
  uint subpass_idx = 0;
  render_pass.CreateSubpass(subpass_idx, gfx::EQueueType::GRAPHICS);
  render_pass.AddColorAttachmentsForSubpass(subpass_idx, api::Attachments{color_idx});
  render_pass.AddDepthAttachmentsForSubpass(subpass_idx, api::Attachment{depth_idx});
  ...
  for (uint i = 0; i < gfx.GetBackBuffersCount(); ++i)
  {
    ...
    auto* pocb_to_read_barrier = GMNew api::BufferBarrier(&device, *pocb_buf, gfx::EAccessFlags::TRANSFER_WRITE, gfx::EAccessFlags::UNIFORM_READ);
    pocb_to_read_barriers.push_back(pocb_to_read_barrier);
    auto* pocb_to_write_barrier = GMNew api::BufferBarrier(&device, *pocb_buf, gfx::EAccessFlags::UNIFORM_READ, gfx::EAccessFlags::TRANSFER_WRITE);
    ...
    auto* pfcb_to_read_barrier = GMNew api::BufferBarrier(&device, *pfcb_buf, gfx::EAccessFlags::TRANSFER_WRITE, gfx::EAccessFlags::UNIFORM_READ);
    pfcb_to_read_barriers.push_back(pfcb_to_read_barrier);
    auto* pfcb_to_write_barrier = GMNew api::BufferBarrier(&device, *pfcb_buf, gfx::EAccessFlags::UNIFORM_READ, gfx::EAccessFlags::TRANSFER_WRITE);
