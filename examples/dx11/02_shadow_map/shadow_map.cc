...
#include "descriptions/vert_sh_buffers.h"
#include "descriptions/frag_sh_buffers.h"
#include "descriptions/input_layout.h"
#include "descriptions/texture_desc.h"
#include "descriptions/rasterizer_desc.h"
#include "descriptions/sampler_state.h"
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE prevInstance, LPWSTR cmdLine, int cmdShow)
{
  ...
  d3d.AddRasterizerState(GDM_HASH("rasterizer_shadow_map"), RasterizerShadowMap());
  d3d.AddRasterizerState(GDM_HASH("rasterizer_wireframe"), RasterizerWireframe());
  DxInput input (win.GetHandle(), hInstance);
  input.SetMouseSensitive(0.2f);

  VertexShader flat_vx {"shaders/flat_vert.hlsl", StdInputLayout{}, d3d};
  DxUniform<FlatVx_PFCB> flat_vx_PFCB (d3d.GetDevice());
  DxUniform<FlatVx_POCB> flat_vx_POCB (d3d.GetDevice());
  PixelShader flat_px {"shaders/flat_frag.hlsl", StdSamplerState{}, d3d};
  DxUniform<FlatPx_POCB> flat_px_POCB (d3d.GetDevice());
  VertexShader text_vx {"shaders/depth_vert.hlsl", StdInputLayout{}, d3d};  // uses the same buffers as in flat
  PixelShader text_px {"shaders/depth_frag.hlsl", StdSamplerState{}, d3d};
  VertexShader plane_vx {"shaders/plane_2d_vert.hlsl", StdInputLayout{}, d3d};
  PixelShader plane_px {"shaders/plane_2d_frag.hlsl", StdSamplerState{}, d3d};
  VertexShader dbg_vx {"shaders/debug_draw_vert.hlsl", DbgInputLayout{}, d3d};
  PixelShader dbg_px {"shaders/debug_draw_frag.hlsl", StdSamplerState{}, d3d};

  CameraEul camera {75.f, win.GetAspectRatio(), 0.1f, 100.f};
  camera.SetPos(Vec3f(0.f, 0.2f, -5.f));
  camera.SetMoveSpeed(2.f);

  DxObject cube(d3d, "../../_models_old/cube_test/cube_test.ply");
  cube.SetPos(Vec3f(0.f, 1.f, 0.f));
  DxObject cube2(d3d, "../../_models_old/cube_test/cube_test.ply");
  cube2.SetTm(matrix::MakeScale(0.5f));
  cube2.SetPos(Vec3f(-2.f, 0.5f, 2.5f));
  DxObject stone(d3d, "../../_models_old/stone_sph/stone.ply");
  stone.SetPos(Vec3f(-10.f, -3.1f, -20.f));
  DxObject plane(d3d, "../../_models_old/plane_xy/plane.ply");
  plane.SetPos(Vec3f(-7.8f, 3.8f));
  DxObject ground(d3d, "../../_models_old/plane_xz/plane.ply");
  ground.SetName(GDM_HASH("ground"));
  ground.SetPos(Vec3f(0.f, 0.f));
  ground.SetTm(matrix::MakeScale(5.f));

  DxObject lamp(d3d, "../../_models_old/sphere_pearl/sphere.ply");
